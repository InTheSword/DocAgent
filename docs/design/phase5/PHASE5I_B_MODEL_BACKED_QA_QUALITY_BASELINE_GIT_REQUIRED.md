...
本任务必须遵守最新 PM 指令：每个阶段实施时新建独立分支，后续收束时再合并；每次任务执行都必须自动执行 Git 检查、分支、提交和同步相关命令。
...
如果该分支已存在，则继续使用该分支；如果不存在，则从当前默认主干创建。

### 0.1.2 禁止事项

- 禁止直接在 `main` / `master` 上实现本任务。
- 禁止自动 merge 到主干。
- 禁止 force push。
- 禁止提交 `.secrets/`、`.env`、API key、模型权重、SQLite 数据库、大型原始数据或大批量 `outputs/` 产物。
- 禁止用 `git reset --hard`、`git clean -fd` 等破坏性命令清理未知变更。
...
如起始工作区存在未知来源的未提交变更，必须停止并返回 `preflight_blocked`，不得覆盖或擅自提交未知变更。
...
git switch main
git pull --ff-only
git switch -c phase5/phase5i-b-model-answer-quality
```
...
```bash
git switch phase5/phase5i-b-model-answer-quality
...
```bash
git push -u origin phase5/phase5i-b-model-answer-quality
```

如果远程分支已经存在：

```bash
git pull --rebase origin phase5/phase5i-b-model-answer-quality
git push origin phase5/phase5i-b-model-answer-quality
```

### 0.1.7 返回报告必须包含 Git 执行报告
