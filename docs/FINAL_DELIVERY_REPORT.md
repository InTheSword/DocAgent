...
| Full SFT/GRPO training | not_started | The v3 schema warmup and checkpoint diagnostic smokes exist, but no full SFT run, GRPO run, or trained-checkpoint quality claim is accepted; later expanded SFT should prefer `ms-swift` after environment preflight and approval for any package installation |
