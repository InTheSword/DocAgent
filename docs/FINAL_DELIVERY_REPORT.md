...
| User-best execution profile | real_model_verified | `scripts/docagent_cli.py` defaults to `--execution-profile user_best`, expanding normal use to MinerU API, LLM query planning, `hybrid_rerank`, BGE-M3, cross-encoder reranker, Qwen SFT AnswerPolicy, the 402-record rejection-SFT checkpoint, and `v3_refs`; server closeout `final_delivery_user_best_profile_closeout_20260706` verified the raw-PDF path with MinerU API submission, saved MinerU Markdown/content-list/resources, BGE/reranker CUDA use, Qwen SFT, citations, evidence_used, and trace artifacts; `formal_benchmark_acceptance=false` |
...
| Final delivery benchmark gate | accepted | Server run `final_delivery_benchmark_gate_server_20260702_componentmetrics` completed readiness, real-Qwen AnswerPolicy baseline, and MP-DocVQA full-workflow diagnostic steps successfully; `final_delivery_gate_metric_review_componentcountfix_20260702` verified manifest/safety flags and complete component-use metrics for 23 local_fact_qa workflow rows; the gate can now pass explicit `--answer-policy sft`, `--adapter-path`, and `--answer-output-contract v3_refs` through both AnswerPolicy baseline and MP-DocVQA workflow diagnostics so candidate checkpoints enter the same execution chain; its preflight blocks requested `cuda*` model devices in AutoDL no-card mode before launching child steps; server run `final_delivery_gate_promptfix_adapter_v3_limit8_20260705` verified the workflow-only path, and `final_delivery_gate_promptfix_adapter_v3_fullsmall_20260705` verified the small complete gate with baseline plus workflow using the promptfix 1024-step adapter and `v3_refs`; artifact review `final_delivery_gate_promptfix_adapter_v3_fullsmall_20260705_contractguard_review_childsummary` verified both model-backed child steps requested and recorded `v3_refs`; follow-up gate `final_delivery_gate_temp095_rejsft402_v3refs_system_subset_20260705` verified the current 402-record rejection-SFT candidate through readiness, AnswerPolicy baseline, and MP-DocVQA workflow, with review confirming manifests, safety flags, complete component-use metrics, and observed `v3_refs`; the gate inspector fails artifact review if model child-step output contracts are missing or mismatched; `formal_benchmark_acceptance=false` |
| Final answer quality artifact contract | real_model_verified | `scripts/run_phase5i_answer_quality_benchmark.py` writes `metrics.json`, `predictions.jsonl`, `case_reports.jsonl`, `failure_analysis.md`, `acceptance_report.json`, `training_candidates_raw.jsonl`, and `manifest.json`; it forwards retriever/dense/reranker settings into `docagent_cli.py` and blocks model-backed runs before CLI/Qwen execution when the requested `db_path` + `doc_id` has no retrievable persisted EvidenceBlocks; server guard run `phase5ib_answer_quality_context_block_20260702` verified that missing context returns blocked artifacts without Qwen; server inventory `phase5ib_context_inventory_server_20260702` found 57 ready candidate contexts and selected `outputs/docagent.db` / `c1fc1c5e040ec894`; follow-up probes added compact CLI/retriever/error/traceback diagnostics, then `phase5ib_answer_quality_selected_context_densefix_20260702` validated stale dense-index metadata rebuild with 8/8 CLI statuses successful; clean-case run `phase5ib_clean6_rejsft402_answer_quality_20260705` exercised the artifact contract with real LLM query rewriting, BGE-M3, reranker, Qwen SFT AnswerPolicy, and `v3_refs`; artifact review passed with safety flags false; remaining failures are answer/evidence keyword diagnostics, not execution-chain blockers |
| Phase 5I answer-quality run compare guard | implemented | `scripts/compare_phase5i_answer_quality_runs.py` compares two existing Phase 5I answer-quality artifact directories, writes case-level movement rows, and keeps training/benchmark safety flags false; local targeted tests passed, server run `phase5ib_v3refs_clean6_base_vs_adapter480_compare_script_20260704` reproduced the clean6 base-vs-adapter result with base 6/6 and adapter480 3/6 without calling models or training, and follow-up run `phase5ib_v3refs_clean6_base_vs_adapter480_boundaryfix_20260704` recorded `promotion_gate.decision=blocked` for default adapter deployment. This guard is a workflow deployment/regression signal, not a standalone judgement of whether the v3 SFT/RL training objective improved |
...
| AnswerPolicy v3 real-model candidate generation | real_model_verified | `scripts/generate_answer_policy_v3_candidates.py` generates multiple `model_generation` candidates from train-only v3 prompts, validates v3 JSON/schema, and writes candidate artifacts for the rejection builder without starting training; local dry-run confirmed synthetic candidates cannot become training-ready, server Qwen smoke `answer_policy_v3_candidate_generation_qwen_smoke_20260704` verified the base path, and promptfix checkpoint runs `answer_policy_v3_candidates_promptfix1024_train256x4_20260705` plus `answer_policy_v3_candidates_promptfix1024_train256x4_offset256_20260705` generated 2048 candidates from two bounded 256-record train-only slices. Follow-up ranking produced 377 rejection-SFT records but only 44 training-ready preference pairs, so DPO/GRPO remain unapproved and simple expansion of the same recipe is not the next default action |
...
| AnswerPolicy v3 ms-swift DPO smoke | real_model_verified | `scripts/run_answer_policy_v3_msswift_dpo.py` converts train-only `training_ready` preference pairs into ms-swift DPO `messages + rejected_response` JSONL, records the exact `swift rlhf --rlhf_type dpo` command, and fixes both `--adapters` and `--ref_adapters` to the source SFT checkpoint. Server run `answer_policy_v3_candidates_temp095_train512x8_offset768_20260705` plus ranking `answer_policy_v3_rejection_temp095_train512x8_offset768_20260705` produced 131 training-ready preference pairs; DPO run `answer_policy_v3_dpo_temp095_pairs128_16steps_20260705` completed 16 steps, but heldout comparison `answer_policy_v3_heldout256_compare_temp095_vs_dpo16_20260705` regressed answer exact 0.6055 -> 0.5977 with zero improvements and two regressions. This verifies DPO plumbing only; it blocks DPO promotion and further DPO tuning without stronger train-only preference data |
| AnswerPolicy v3 custom GRPO smoke | real_model_verified | `scripts/run_answer_policy_v3_custom_grpo.py` converts train-only v3 SFT records to prompt-only GRPO records and scores generated completions with the calibrated v3 reward. Server smoke `answer_policy_v3_grpo_smoke4_ng2_from_rejsft402_20260705` completed from the 402-record rejection-SFT checkpoint but had `nonzero_reward_std_steps=0`, so heldout256 was identical to the source checkpoint. Diversity probe `answer_policy_v3_grpo_variance_probe8_ng3_temp12_20260705` produced nonzero reward variance on 3/8 steps, but heldout256 regressed answer exact 0.6133 -> 0.6055 with zero improvements and two regressions. A hard-sample probe selected 38 train-only rows with reward spread >= 0.3; the resource-safe run `answer_policy_v3_grpo_hardsample38_ng3_lr1e7_20260705` completed 16 steps with reward variance but regressed heldout256 to 0.5977 with zero improvements and four regressions. This verifies GRPO execution plumbing only and blocks GRPO checkpoint promotion under the current reward/data/checkpoint recipe |
...
| AnswerPolicy v3 expanded mixed SFT diagnostic | real_model_verified | Server materialization `mpdocvqa_train_evidence_api_400_reuse233_20260704` reached 400/400 passed MP-DocVQA train document windows and 1467/1469 evidence-ready QA samples, then `mpdocvqa_train_evidence_api_848_resume400_20260705` reached 848/848 passed train document windows with 3036/3039 evidence-ready QA samples. The expanded train-only sources produced 2156 MP-DocVQA supported records, 2000 MP-DocVQA insufficient records, 5000 TAT-QA supported records, and 2000 TAT-QA insufficient records. Mixed pack `answer_policy_v3_mixed_stage2_full4096_20260705` selected 4096 records without shortage/backfill. Promptfix SFT run `answer_policy_v3_msswift_stage2_promptfix3840_1024steps_20260705` trained Qwen3-1.7B LoRA for 1024 optimizer update steps; train-only heldout answer exact improved 0.3750 -> 0.5859, support/ref metrics improved, and the focused table/calculation fixed-evidence comparator improved answer exact 0.5234 -> 0.7891 with 36 improvements and 2 regressions. Follow-up continuation `answer_policy_v3_msswift_tablecalc_continue1984_384steps_20260705` trained on 1984 train-only table/calculation records and improved strict table/calculation heldout answer exact over promptfix 0.7519 -> 0.7669, but only moved broader heldout256 0.5859 -> 0.5898, regressed insufficient empty-ref behavior, and clean6 remained 4/6 versus base 6/6. This verifies the v3 AnswerPolicy objective on train-only diagnostics, not formal benchmark acceptance or default checkpoint promotion |
...
| VLM visual evidence support | implemented | Native caption attachment, optional VLM visual summary/review, cache metadata, CLI flags, and workflow tool-result wiring are implemented. Server target tests pass, but real VLM API smoke is blocked by the current `.secrets/vlm.env` model/endpoint returning `InvalidParameter` for text+image chat input; this is not `real_model_verified` until a compatible chat-multimodal VLM endpoint is configured |
...
- `phase5ib_clean6_rejsft402_answer_quality_20260705`
- `phase5ib_clean6_rejsft402_answer_quality_20260705_review`
...
Clean-case answer-quality diagnostic
`phase5ib_clean6_rejsft402_answer_quality_20260705` then exercised the current
402-record AnswerPolicy v3 checkpoint through the same real-model path on 6
curated clean answer-quality cases. Artifact review passed; the run preserved
JSON/artifact/citation contracts and safety flags, but passed only 3/6 cases.
This is useful small-scenario answer-quality evidence, not formal benchmark
...
- Image/table evidence is available through OCR text, captions, table HTML,
  ...
  summaries/reviews. Pixel-level VLM interpretation remains unaccepted until a
  compatible VLM endpoint passes server smoke.
