...
| Production SFT/GRPO promotion | not_started | The v3 PEFT warmup, ms-swift 1-step smoke, mixed-pack 3-step smoke, rejection-SFT smoke, 400-document 480-step diagnostic, full4096 promptfix 1024-step SFT diagnostic, promptfix-based 182-record/377-record rejection-SFT continuations, high-diversity temp0.95 candidate generation, and a 201-record temp0.95 continuation exist. They verify training-chain execution and train-only objective improvement. The 377-record continuation improved heldout256 answer exact 0.5859 -> 0.6016 and restored JSON/schema/ref legality plus insufficient empty-ref behavior to 1.0; the temp0.95 continuation improved heldout256 answer exact further to 0.6055 and kept schema/ref legality at 1.0, but insufficient empty-ref behavior returned to 0.9474. Clean6 guards preserve execution/citation flow but still block default deployment without broader evidence. Candidate-pool preference coverage remains low, so DPO and GRPO remain unapproved, no adapter is promoted as the default production AnswerPolicy, and no formal benchmark quality claim is accepted |
