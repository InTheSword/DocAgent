...
| AnswerPolicy v3 mixed-pack short SFT | real_model_verified | `scripts/build_answer_policy_v3_mixed_sft_pack.py` builds train-only mixed packs with ratio, shortage, and backfill audit; server run `answer_policy_v3_mixed_stage2_20260704` selected 64 records, recording MP-DocVQA shortage and TAT-QA backfill, `answer_policy_v3_msswift_stage2_short_20260704` trained Qwen3-1.7B for 3 ms-swift LoRA steps, and `answer_policy_v3_msswift_stage2_checkpoint_eval_20260704` verified JSON/schema/support-ref contract on 8 records; after adding insufficient records, prompt-limit repair run `answer_policy_v3_promptlimit_stage2_short_20260704_verify` rebuilt a 64-record mixed pack with 19 insufficient records, trained 3 ms-swift LoRA steps, and restored `json_valid_rate=1.0`, `schema_valid_rate=1.0`, and `supporting_refs_subset_rate=1.0`; this is short training-chain evidence, not final model-quality acceptance |
