...
| AnswerPolicy v3 training-data trial | implemented | `ModelOutputV3` targets `answer`, `supporting_refs`, `support_status`, and `reasoning_summary`; `scripts/build_answer_policy_v3_training_data.py` builds high-confidence TAT-QA train-only v3 SFT trial artifacts with internal `EvidenceRefMap` metadata and validation-source blocking; local smoke `answer_policy_v3_tatqa_trial_20260704` produced 200 records and did not start training |
