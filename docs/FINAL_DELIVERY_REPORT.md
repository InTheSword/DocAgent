...
| AnswerPolicy v3 adapter-backed rejection distillation artifacts | real_model_verified | Server run `answer_policy_v3_candidate_generation_expand20_adapter_smoke_20260704` loaded the short expanded ms-swift LoRA adapter, generated 24 adapter-backed candidates from 8 train-only records with JSON/schema rates 1.0, and `answer_policy_v3_rejection_sampling_expand20_adapter_smoke_20260704` selected 2 reward-qualified rows into `rejection_sft_candidates.jsonl`; dry-run `answer_policy_v3_rejection_sft_msswift_dryrun_20260704` verified those 2 rows are valid ms-swift SFT input without executing training; bounded follow-up `answer_policy_v3_candidate_generation_expand20_adapter24x3_20260704` generated 72 candidates from 24 train-only records with JSON/schema rates 1.0, but ranking `answer_policy_v3_rejection_sampling_expand20_adapter24x3_20260704` again produced only 2 training-ready rejection-SFT rows and zero training-ready preference pairs; `--offset` support now allows distinct train-only slices, and offset-slice run `answer_policy_v3_candidate_generation_expand20_adapter_offset24x3_20260704` / `answer_policy_v3_rejection_sampling_expand20_adapter_offset24x3_20260704` generated 72 candidates from records 24-47 and selected 11 training-ready rejection-SFT rows, but still zero training-ready preference pairs; DPO/GRPO remain unapproved |
| AnswerPolicy v3 rejection-SFT distillation smoke | real_model_verified | Server run `answer_policy_v3_rejection_sft_msswift_execute_smoke_20260704` trained Qwen3-1.7B for 1 ms-swift LoRA step on the first 2 train-only rejection-SFT rows selected by the adapter-backed rejection sampler, then `answer_policy_v3_rejection_sft_checkpoint_eval_20260704` verified JSON/schema/supporting-ref/support-status/positive-ref metrics at 1.0 on those same rows; follow-up run `answer_policy_v3_rejection_sft_adapter_offset24x3_short_20260704` trained 2 ms-swift LoRA steps on 8 of 11 train-only offset-slice rejection-SFT rows, and `answer_policy_v3_rejection_sft_adapter_offset24x3_checkpoint_eval_20260704` evaluated 8 rows with JSON/schema/ref/status/answer-exact metrics at 1.0; this verifies execution plumbing only, not final answer-quality improvement |
