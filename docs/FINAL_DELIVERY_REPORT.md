...
| Production SFT/GRPO promotion | not_started | The v3 PEFT warmup, ms-swift 1-step smoke, mixed-pack 3-step smoke, rejection-SFT smoke, 400-document 480-step diagnostic, full4096 promptfix 1024-step SFT diagnostic, promptfix-based 182-record and 377-record bounded rejection-SFT continuations, and 512-record promptfix candidate-pool review exist. They verify training-chain execution and train-only objective improvement. The 377-record continuation improved heldout256 answer exact 0.5859 -> 0.6016 and restored JSON/schema/ref legality plus insufficient empty-ref behavior to 1.0, but slightly reduced positive-ref hit and failed the clean6 default-deployment guard versus the compared adapter run. The candidate pool produced only 44 training-ready preference pairs; DPO and GRPO remain unapproved, no adapter is promoted as the default production AnswerPolicy, and no formal benchmark quality claim is accepted |
