...
| AnswerPolicy v3 insufficient-evidence data | implemented | `scripts/build_answer_policy_v3_insufficient_data.py` builds train-only TAT-QA `insufficient_confirmed` records by pairing source questions with different-document decoy evidence boards that do not contain the gold answer string; local smoke `answer_policy_v3_tatqa_insufficient_local_smoke_20260704` produced 32 v3 records with `support_status=insufficient`, empty `supporting_refs`, and validation-source blocking; no training was started |
...
| AnswerPolicy v3 mixed-pack short SFT | real_model_verified | `scripts/build_answer_policy_v3_mixed_sft_pack.py` builds train-only mixed packs with ratio, shortage, and backfill audit; server run `answer_policy_v3_mixed_stage2_20260704` selected 64 records, recording MP-DocVQA shortage and TAT-QA backfill, `answer_policy_v3_msswift_stage2_short_20260704` trained Qwen3-1.7B for 3 ms-swift LoRA steps, and `answer_policy_v3_msswift_stage2_checkpoint_eval_20260704` verified JSON/schema/support-ref contract on 8 records with `positive_ref_hit_rate=0.875` and `answer_exact_rate=0.5`; this is short training-chain evidence, not final model-quality acceptance |
