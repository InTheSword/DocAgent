...
M1-G1 已达到 `mock_verified`：同页显式/组合表题关联、大表结构化父表与行组检索
子块、结构化父表接线及质量报告已经实现，相关本地回归 104 项通过。正文切分和查询
链未修改。

当前停止。下一里程碑 M1-G2 是在服务器使用既有 MinerU JSON 受控重建六文档 Chunk
并生成新旧质量对照；该步骤不需要 GPU，但不得在本批自动执行。最终 qrels 仍需等
新 Chunk corpus 冻结后再生成和复核。
