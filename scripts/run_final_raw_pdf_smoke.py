...
def _first_ingestion_failures(payload: dict[str, Any], *, document_root: Path) -> list[str]:
    ...
    if source.get("parser") != "mineru_api":
        failures.append(f"source_parser_mismatch:{source.get('parser')}!=mineru_api")
    if source.get("parser_mode") != "parse_existing":
        failures.append(f"source_parser_mode_mismatch:{source.get('parser_mode')}!=parse_existing")
    ...
    if api_summary.get("api_status") not in {"submitted", "cached", "cached_existing_output"}:
...
def run_final_raw_pdf_smoke(
    *,
    ...
    if not live_api:
        raise ValueError("final raw PDF MinerU API smoke requires --live-api")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--parser", choices=["mineru_api"], default="mineru_api")
