from __future__ import annotations

import json
from pathlib import Path

from docagent.utils.jsonl import read_jsonl, write_jsonl
from scripts.build_answer_policy_v3_rejection_sampling_artifacts import build_rejection_sampling_artifacts


def _record(record_id: str, *, status: str = "supported") -> dict:
    target = {
        "answer": "March 12, 2020" if status == "supported" else "Insufficient evidence.",
        "supporting_refs": ["E1"] if status == "supported" else [],
        "support_status": status,
        "reasoning_summary": "The selected evidence contains the answer."
        if status == "supported"
        else "No candidate contains the answer.",
    }
    return {
        "id": record_id,
        "source": "tatqa",
        "messages": [
            {
                "role": "system",
                "content": "Return exactly one valid JSON object with answer, supporting_refs, support_status, and reasoning_summary.",
            },
            {
                "role": "user",
                "content": "## Question\nWhat is the invoice date?\n\n## Evidence Candidates\n[E1] Invoice Date: March 12, 2020\n[E2] Vendor: Acme",
            },
            {"role": "assistant", "content": json.dumps(target)},
        ],
    }


def test_rejection_sampling_artifacts_select_real_candidate(tmp_path: Path) -> None:
    sft_path = tmp_path / "train" / "sft_train.jsonl"
    candidate_path = tmp_path / "train" / "candidates.jsonl"
    record = _record("q1")
    target = json.loads(record["messages"][-1]["content"])
    write_jsonl(sft_path, [record])
    write_jsonl(
        candidate_path,
        [
            {
                "id": "q1",
                "candidates": [
                    {"candidate_id": "bad", "prediction": {**target, "answer": "Unsupported answer."}},
                    {"candidate_id": "good", "prediction": target},
                ],
            }
        ],
    )

    result = build_rejection_sampling_artifacts(
        sft_inputs=[sft_path],
        candidate_inputs=[candidate_path],
        output_root=tmp_path / "out",
        run_id="rs",
    )

    artifact_dir = tmp_path / "out" / "rs"
    selected = read_jsonl(artifact_dir / "selected_candidates.jsonl")
    pairs = read_jsonl(artifact_dir / "preference_pairs.jsonl")
    rejection_sft = read_jsonl(artifact_dir / "rejection_sft_candidates.jsonl")

    assert result["status"] == "success"
    assert result["used_training"] is False
    assert result["used_qwen"] is False
    assert result["validation_subset_used_for_training"] is False
    assert result["metrics"]["training_ready_selected_count"] == 1
    assert result["metrics"]["training_ready_preference_pair_count"] == 1
    assert selected[0]["chosen_candidate"]["candidate_id"] == "good"
    assert selected[0]["training_ready"] is True
    assert pairs[0]["reward_margin"] >= 0.2
    assert pairs[0]["training_ready"] is True
    assert rejection_sft[0]["messages"][-1]["role"] == "assistant"
    assert (artifact_dir / "summary.md").is_file()


def test_rejection_sampling_calibration_variants_are_not_training_ready(tmp_path: Path) -> None:
    sft_path = tmp_path / "train" / "sft_train.jsonl"
    write_jsonl(sft_path, [_record("q1"), _record("q2", status="insufficient")])

    result = build_rejection_sampling_artifacts(
        sft_inputs=[sft_path],
        output_root=tmp_path / "out",
        run_id="calibration",
        use_calibration_variants=True,
    )

    artifact_dir = tmp_path / "out" / "calibration"
    selected = read_jsonl(artifact_dir / "selected_candidates.jsonl")
    rejection_sft = read_jsonl(artifact_dir / "rejection_sft_candidates.jsonl")

    assert result["status"] == "success"
    assert result["metrics"]["source_mode"] == "calibration_variants"
    assert result["metrics"]["candidate_source_counts"] == {"calibration_variant": 9}
    assert result["metrics"]["training_ready_selected_count"] == 0
    assert result["metrics"]["rejection_sft_record_count"] == 0
    assert selected[0]["training_ready"] is False
    assert "synthetic_candidate_source" in selected[0]["not_training_ready_reasons"]
    assert rejection_sft == []


def test_rejection_sampling_blocks_validation_like_input(tmp_path: Path) -> None:
    sft_path = tmp_path / "final_eval" / "sft_train.jsonl"
    write_jsonl(sft_path, [_record("q1")])

    result = build_rejection_sampling_artifacts(
        sft_inputs=[sft_path],
        output_root=tmp_path / "out",
        run_id="blocked",
        use_calibration_variants=True,
    )

    assert result["status"] == "blocked"
    assert result["block_reasons"] == [f"validation_like_input_path:{sft_path.as_posix()}:final_eval"]
    assert read_jsonl(tmp_path / "out" / "blocked" / "ranked_candidates.jsonl") == []
