...
def test_mineru_api_client_reads_token_and_writes_sanitized_manifest(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert fake.post_payloads[0]["files"] == [
        {"name": "sample.pdf", "data_id": "sample_data", "is_ocr": True}
    ]
    ...
    assert manifest["parse_options"]["is_ocr"] is True
    assert manifest["submission_payload"]["files"][0]["is_ocr"] is True
